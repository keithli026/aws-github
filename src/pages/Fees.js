...
const Fees = () => {
  ...
  useEffect(() => {
    ...
    const fetchFeesData = async () => {
      try {
        const response = await fetch(`/locales/${language}/fees.json`);
        if (!response.ok) {
  ...
  return (
    <Container id="fee">
      ...
      <div className="table-responsive mb-5">
        <table className="table table-bordered table-hover">
          <thead>
            <tr>
              <th></th>
              <th>{t('fees:home_visit_physiotherapy_consultation_fee.headers.time')}</th>
              <th>{t('fees:home_visit_physiotherapy_consultation_fee.headers.fee')}</th>
            </tr>
          </thead>
          <tbody>
      ...
      <h2>{t('fees:physiotherapy_program_and_package_fees.title')}</h2>
      <div className="table-responsive mb-5">
        <table className="table table-bordered table-hover">
          <thead>
            <tr>
              <th>{t('fees:physiotherapy_program_and_package_fees.headers.package_program')}</th>
              <th>{t('fees:physiotherapy_program_and_package_fees.headers.description')}</th>
              <th>{t('fees:physiotherapy_program_and_package_fees.headers.time')}</th>
              <th>{t('fees:physiotherapy_program_and_package_fees.headers.fee')}</th>
            </tr>
          </thead>
          <tbody>
      ...
      <h2>{t('fees:home_visit_massage_fee.title')}</h2>
      <div className="table-responsive mb-5">
        <table className="table table-bordered table-hover">
          <thead>
            <tr>
              <th></th>
              <th>{t('fees:home_visit_massage_fee.headers.time')}</th>
              <th>{t('fees:home_visit_massage_fee.headers.fee')}</th>
            </tr>
          </thead>
          <tbody>
